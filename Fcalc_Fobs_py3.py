...
class Main_Window(QMainWindow):
	...
	def plot_Fcalc_Fobs(self):
		...
		if self.cb1_1.checkState():#1 get Fobs from mtz file
			...
			model = dict(list(fcf_file.file_content().model().items())[0][1])
	...
	#-------------------------------------------------------------------------------
		...
		af = AnnoteFinder(merged_fobs,merged_fcalc, list(zip(index,ds)), ax=ax)
		fig.canvas.mpl_connect('button_press_event', af)
		self.fit_window = FitWindow(ax=ax,data=list(zip(merged_fobs,merged_fcalc)))
...
class FitWindow(QMainWindow):
	...
	def fit(self):
		...
		data1,data2 = list(zip(*self.data))
		data1 = np.array(data1)
		data1 = data1[~np.isnan(data1)]
		data2 = np.array(data2)
		data2 = data2[~np.isnan(data2)]
		x = np.array(list(range(int(data1.min()),int(data1.max())))).reshape(-1,1)
		pred = regr.predict(x)
